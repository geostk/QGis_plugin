...
TABLE_MIMETYPES = [{"MIMETYPE":"text/csv", "SCHEMA":"", "GDALID":"CSV", "DATATYPE":"CSV", "EXTENSION":"csv"}]
...
def getSupportedMimeTypes(inElement):
    mimeTypes = [getDefaultMimeType(inElement)]
...
def isMimeTypeTable(mimeType, ignorePlaylist = False):
    """Check for vector input. Zipped shapefiles must be extracted"""
    if not ignorePlaylist:
      if isMimeTypePlaylist(mimeType) != None:
        return None

    for tableType in TABLE_MIMETYPES:
        if tableType["MIMETYPE"] in mimeType.lower():
          return tableType["GDALID"]
    return None
...
def getFileExtension(mimeType):
    ...
    elif isMimeTypeTable(mimeType):
      for tableType in TABLE_MIMETYPES:
          if tableType["MIMETYPE"] in mimeType.lower():
            return tableType["EXTENSION"]
