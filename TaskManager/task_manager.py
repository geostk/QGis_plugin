...
class TaskManager:
    ...
    def __init__(self, iface):
        ...
        self.actions = []
        self.menu = self.tr(u'&Task_Manager')
    ...
    def run(self):
        ...
        #self.dlg.comboBoxTaskStatus.clear()
        #self.dlg.createByComboBox.clear()
        if self.dlg.isVisible():
           ...
           self.apdockwidget.close()
            ...
            # self.apdockwidget.setObjectName("TaskManager")
            self.apdockwidget.setWindowFlags(Qt.FramelessWindowHint)

            self.apdockwidget.setWidget(self.dlg)
            self.iface.addDockWidget(Qt.LeftDockWidgetArea, self.apdockwidget)
            self.dlg.populate()


        # rows = self.dlg.taskListTable.rowCount()

        # createdByList=[]



        # # for i in len(createdByList):
        # #     if createdByList[i]==createdByList[]:
        # #



        # taskStatusList=['--Select--','Initiated','In Process','Canceled','Finished']
        # self.dlg.comboBoxTaskStatus.addItems(taskStatusList)
        # for row in range(0,rows):



            # createdByItems=self.dlg.taskListTable.item(row,1)
            # createdByList.append(createdByItems.text())

        # # createdByListSize=len(createdByList)
        # createdByList.append(str("--Select--"))
        # createdByList=list(set(createdByList))



        # # for i in range(0,createdByListSize):
        # #     if createdByList[0]==createdByList[i]:
        # #         del createdByList[i]

        #self.dlg.createByComboBox.addItems("agsdj")
